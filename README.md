...
- [x] 사용자는 다음 명령어를 입력할 수 있다.
    ...
    - [x] '빌드' : 명언 모음 파일(data.json)을 생성하는 명령어
...
- [x] '빌드' 명령어 입력 시 'data.json의 내용이 갱신되었습니다.'를 출력한다.
...
- [x] 명언 데이터는 애플리케이션이 종료된 이후에도 유지되어야 한다.
    - [x] 각 명언은 개별 파일로 db/wiseSaving 폴더에 저장된다.
    - [x] 각 명언의 파일명은 {명언번호}.json 형식으로 저장된다. 
      (ex. 1.json, 2.json, ...)
- [x] 각 명언 데이터의 파일 내용은 다음과 같다.
  (ex. 1.json)
    ```json
    {
      "id": 1,
    ...
    ```
- [x] 가장 마지막에 생성된 명언 번호는 db/wiseSaving/lastid.txt 파일에 저장된다.
    - [x] 애플리케이션 시작 시 lastid.txt 파일을 읽어와서 마지막 명언 번호를 복원한다.
    - [x] last_id.txt 파일이 존재하지 않을 경우 마지막 명언 번호는 0으로 간주한다.
- [x] 명언을 등록, 수정할 때 해당 파일이 갱신되어야 한다.

### <span style="color:#e67e22">명언 모음</span>
- [x] '빌드' 명령어 입력 시 모든 명언들을 모아서 data.json 파일로 생성한다.
    - [x] data.json 파일의 내용은 다음과 같다.
    ```json
    [
      {
        "id": 1,
